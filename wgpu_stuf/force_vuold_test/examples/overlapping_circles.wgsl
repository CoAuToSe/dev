...
fn fs_main(in: VertexOutput) -> [[location(0)]] vec4<f32> {
    ...
    for (var i: f32 = 1.; i < 500.; i = i + 1.) {
        let rx: f32 = fract(sin(i) * 1000.);
        let ry: f32 = fract(sin(i) * 1001.);
        let sx: f32 = fract(sin(i) * 1009.) - 0.5;
        let sy: f32 = fract(sin(i) * 1011.);

        let xx = fract(rx + sx * uniforms.time / 4.) * 2.4 - 1.2;
        let yy = fract(ry + sy * uniforms.time / 4.) * 2.4 - 1.2;

        let r = (fract(sin(i) * 1044.) + 1. / 2.) * 0.06 + 0.01;
