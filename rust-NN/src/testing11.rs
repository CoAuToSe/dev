...
// Network now contains a Vec of Layers
struct Network {
    layers: Vec<Layer>,
}

impl Network {
    fn new(layer_config: Vec<((usize, usize), usize, ActivationFunction)>) -> Self {
        let layers = layer_config
            .into_iter()
            .map(|(weight_dims, bias_dim, activation)| {
                Layer::new(weight_dims, bias_dim, activation)
            })
            .collect();
        Network { layers }
    }
    ...
    fn backward(&mut self, x: &Array2<f64>, grad: &Array2<f64>) -> Vec<(Array2<f64>, Array2<f64>)> {
        self.layers
            .iter_mut()
            .fold(vec![(x.clone(), grad.clone())], |mut acc, layer| {
                let (x, grad) = acc.last().unwrap();
                acc.push(layer.backward(x, grad));
                acc
            })
    }
}

// train function now handles multiple layers
fn train(
    net: &mut Network,
    opt: &mut Optimizer,
    ...
    for epoch in 1..=epochs {
        ...
        let grad = &loss_grad * &net.layers.last().unwrap().activation.derivative(&y_pred);
        let grads = net.backward(&x_train, &grad);

        for (layer, (dw, db)) in net.layers.iter_mut().zip(grads.iter()) {
...
fn main() {
    ...
    let mut net = Network::new(vec![
        ((10, 10), 10, ActivationFunction::Sigmoid),
        ((10, 1), 1, ActivationFunction::Sigmoid),
    ]);
