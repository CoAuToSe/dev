...
{  
 ...
 //Column main element Gaussian elimination method  
 //A is the coefficient matrix and x is the solution vector. If successful, return true, otherwise return false, and clear x.  
...
{  
    ...
    int m = A[0].size();  
    x.resize(n);  
         //Copy the coefficient matrix to prevent modifying the original matrix  
    ...
    {  
        ...
        if (l != k)  
        {  
                         //Exchange the l row and k row of the coefficient matrix  
            for (int i = 0; i < m; i++)  
            {  
...
{     
    ...
    float *x1=new float[5];  
         //Solve the least squares solution of Ax^2+By^2+Cxy+Dx+Ey=10000!  
    SVD(A1,n,5,B1,x1,min_eps);  
    A2[0]=2*x1[0],A2[1]=A2[2]=x1[2],A2[3]=2*x1[1];  
    B2[0]=x1[3],B2[1]=x1[4];  
    float *x2=new float[2];  
         //Standardize, eliminate one item, find center.x and center.y;  
    ...
    {  
        ...
        A3[step+2] = (px - rp[0]) * (py - rp[1]);  
         ...
         //Find the least square solution of A(x-center.x)^2+B(y-center.y)^2+C(x-center.x)(y-center.y)  
