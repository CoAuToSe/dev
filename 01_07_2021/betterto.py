...
def gradient_descent(_x, _y, _z, çbeta, elipse, state , çfig, àax, _learningrate = 0, _epochs=5):
    ...
    # print(arra)
    # trace = pd.DataFrame(arra, columns=['A', 'B', 'C', 'D', 'E', 'F', 'my_delta'])
    # trace.replace([0, 1, 2, 3], 1)
    print(arra)
    x = np.array(_x)
    y = np.array(_y)
    z = np.array(_z)
    ...
    #                                   index=[str(0)]))
    # trace['A'] = pd.DataFrame(data=[A, B, C, D, E, F, my_delta.sum()])
    # trace.sub(pd.Series([1, 1, 1], index=['circle', 'triangle', 'rectangle'])
    # trace.sub([A, B, C, D, E, F, my_delta.sum()], axis='columns', level='11')
    # for i in range(0, 7):
    #    axis='index')
        
    for E,e in enumerate([i, A, B, C, D, E, F, my_delta.sum()]):
        arra[E][0] = e 
    ...
    for i in range(_epochs):
        ...
        for E,e in enumerate([i, A, B, C, D, E, F, my_delta.sum()]):
            arra[E][i+1] = e 
        # trace[i+1] = pd.DataFrame(data=[[A, B, C, D, E, F, my_delta.sum()]], 
        #                                   columns=['A', 'B', 'C', 'D', 'E', 'F', 'my_delta'], 
        #                                   index=['' + str(i+1)])
        # trace[i+1] = pd.DataFrame(data=[A, B, C, D, E, F, my_delta.sum()])
...
#   çax.draw_artist(list_to_show[-1][1][0])
...
#   some[3].relim()
#   some[3].autoscale_view()
#   some[3].set_array(some[4])
#   some[3].set_offsets(some[1][0].get_)
#   some[3].set_xdata(e[4][0])
#   some[3].set_ydata(e[4][1])
  some[2].relim()
...
def graph_plot_var(çfig, çax, çname_var, çarra,çname, pos = [0,0,0]):
  ...
  new_ax.set_title(çname_var)
#   çax.set_xticks([])
  # çax.yticks([])
#   çax.set_autoscaley_on(True)
...
#   plt.style.use(['dark_background'])
  list_to_show.append([çname, new_ax.plot(çarra[0], çarra[çname_var], 'r-o', animated=True), new_ax, çname_var])
...
def updateResult(_A, _B, _C, _D, _E, _F, _my_delta, _arra, Xvalues, Yvalues, fig, ax ,_Elips, çbeta, bg):
    ...
    for e in list_to_show:
        ...
        if e[0] == "var":
            ...
            # print(_trace[e[3]])
            # _trace[e[3]].plot.line(subplots=True)
            # if len(_trace[e[3]]) >= 10: 
            #     # print(e, e[3])
            #     e[1][0].set_xdata(_trace[e[3]]) 
            #     e[2].draw_artist(e[1][0])
...
def do_show(çalpha, çbeta, çnum_step, çnum_var, çrandi, çstating, learningrate = 0.05, _state=1):
    ...
    for E,e in enumerate(çarra):
        print(params[E], ": | begin: ", e[0], "| result: ",e[çnum_step], " | goal: ", _Elip, "|", çbeta)
    # print()
    plt.show(block = True)
