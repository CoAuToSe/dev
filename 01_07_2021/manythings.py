...
def gradient_descent(_x, _y, _z, çbeta, elipse, state , çfig, àax, _learningrate, _epochs=5):
    ...
    for r in [A,B,C,D,E,F,my_delta.sum(), my_delta2.sum()]:
        temp_arra = []
    ...
    for H,e in enumerate([i, A, B, C, D, E, F, my_delta.sum(), my_delta2.sum()]):
        arra[H][0] = e 
    ...
    for i in range(1,_epochs+1):
        ...
        # if (abs(my_delta.sum()) >= abs(lowest[0])) and (abs(my_delta2.sum()) >= abs(lowest[1])) and i >= 10 : print("\n",i);max_index=i;break
        if abs(my_delta.sum()) <= abs(lowest[0]): lowest=[my_delta.sum(),my_delta2.sum(),i,A,B,C,D,E,F]
...
def do_show(çalpha, çbeta, çnum_step, çnum_var, çrandi, çstating, learningrate = 0.05, _state=1):
    ...
    # print(çarra)
    sor_elli_val = [çnum_step, _Elip[1], _Elip[3], _Elip[5], _Elip[0], _Elip[2], _Elip[4], 0, 0, "", ""]
