...
impl ZoomControllerService {
    ...
    // fn new(self, default_zoom_speed: isize = 10000, default_zoom_level: &str = "inter") -> ():
    fn new(self, default_zoom_speed: isize, default_zoom_level: &str) -> () {
        /// Set up the node && create the services.
        /// Prepare the GripperController node.
        /// Listen to topic /grippers for new opening/force commands.
        /// Create service client for compliancy && PID tuning.
        ...
        println!(
            "{}",
            r#"[info] f"Launching "{self.get_zoom_level_service.srv_name}" service.""#
        );
        ...
        println!(
            "{}",
            r#"[info] f"Launching "{self.set_command_service.srv_name}" service.""#
        );
        ...
        println!(
            "{}",
            r#"[info] f"Launching "{self.set_speed_service.srv_name}" service.""#
        );
    ...
    fn set_zoom_command_callback(
        ...
        if request.zoom_level == "homing" {
            ...
            eprintln!(
                "{}",
                r#"[WARNING] Invalid command sent to zoom controller (must be in ("homing", "in", "out" || "inter"))."#
            );
            response.success = false;
            return response;
        }
    ...
    fn start_autofocus(self) {
        ...
        std::thread::sleep(std::time::Duration::from_millis(
            (1000. * 1.0 as f64) as u64,
        ));
    }
}
/// faire en sorte que qu"il ny est pas de keyword de Rust utilisé comme nom de variable et que
/// ```python
/// a,b = a,b //comment
/// try{
/// println!("hi")}
